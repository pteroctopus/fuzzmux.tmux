...
#   --sessions          the one-row-per-session list, restricted to the session
#                       ids in $FUZZMUX_CH_LIMIT when that file exists
#   --limit <file>      remember the sessions of the rows fzf selected ({+f})
#   --toggle            fzf transform: switch text <-> sessions mode
#   --all               fzf transform: lift the restriction in sessions mode
...
PROMPT_LIMITED="sessions (from text search) > "
...
# --- fzf helpers: session list, restriction, mode switches ---------------------------

# The one-row-per-session list, restricted to the ids in the limit file if any.
if [[ "${1:-}" == "--sessions" ]]; then
  limit="${FUZZMUX_CH_LIMIT:-}"
  if [[ -n "$limit" && -s "$limit" ]]; then
    awk -F "$BAR" 'NR == FNR { keep[$1] = 1; next } ($1 in keep)' "$limit" "${FUZZMUX_CH_ROWS:?}"
  else
    cat "${FUZZMUX_CH_ROWS:?}"
  fi
  exit 0
...
# Remember which sessions the rows fzf handed over ({+f}: one row per line)
# belong to, so the sessions list can be restricted to them.
if [[ "${1:-}" == "--limit" ]]; then
  [[ -n "${2:-}" && -f "$2" && -n "${FUZZMUX_CH_LIMIT:-}" ]] || exit 0
  cut -d "$BAR" -f 1 "$2" | sort -u >"$FUZZMUX_CH_LIMIT"
  exit 0
fi

# Actions for the filter key. From text mode: go to the sessions list, limited
# to the sessions of the current text matches when a query is typed. From
# sessions mode: back to the full text list. Both start with a clean query.
if [[ "${1:-}" == "--toggle" ]]; then
  self="$(printf '%q' "$0")"
  if [[ "${FZF_PROMPT:-}" == "$PROMPT_TEXT" ]]; then
    if [[ -n "${FZF_QUERY:-}" ]]; then
      printf 'select-all+execute-silent(%s --limit {+f})+deselect-all+change-prompt(%s)+change-header(%s)+reload(%s --sessions)+change-nth(1..)+clear-query+first+refresh-preview\n' \
        "$self" "$PROMPT_LIMITED" "${FUZZMUX_CH_HEADER_SESSIONS:-}" "$self"
    else
      rm -f "${FUZZMUX_CH_LIMIT:-/nonexistent}"
      printf 'change-prompt(%s)+change-header(%s)+reload(%s --sessions)+change-nth(1..)+clear-query+first+refresh-preview\n' \
        "$PROMPT_SESSIONS" "${FUZZMUX_CH_HEADER_SESSIONS:-}" "$self"
    fi
  else
    printf 'change-prompt(%s)+change-header(%s)+reload(cat %q)+change-nth(2..)+clear-query+first+refresh-preview\n' \
      "$PROMPT_TEXT" "${FUZZMUX_CH_HEADER_TEXT:-}" "${FUZZMUX_CH_TEXT:?}"
  fi
  exit 0
fi

# Actions for the "all sessions" key: only meaningful in sessions mode.
if [[ "${1:-}" == "--all" ]]; then
  self="$(printf '%q' "$0")"
  case "${FZF_PROMPT:-}" in
  "$PROMPT_TEXT") ;;
  *)
    rm -f "${FUZZMUX_CH_LIMIT:-/nonexistent}"
    printf 'change-prompt(%s)+reload(%s --sessions)+first+refresh-preview\n' "$PROMPT_SESSIONS" "$self"
    ;;
  esac
  exit 0
fi
...
META="$WORK/meta"
LIMIT="$WORK/limit"     # session ids the sessions list is restricted to
: >"$ROWS"
: >"$TEXT_ROWS"
: >"$META"
...
while IFS="$DEL" read -r sid last_ts project _count title _prompts; do
  ...
  [[ -n "$title" ]] || title="-"
  ((${#title} > 90)) && title="${title:0:89}${ELLIPSIS}"
  ...
  # A session row carries metadata only (state, age, project, first prompt).
  state_ansi_into color "$state"
  pad_into c_state "$state" 10
  pad_into c_age "$age" 4
  pad_into c_dir "$dir" 38
  printf '%s%s-%s%s%s%s  %s  %s%s %s\n' "$sid" "$BAR" "$BAR" "$color" "$c_state" "${color:+$RESET}" "$c_age" "$c_dir" "$BAR" "$title" >>"$ROWS"
...
export FUZZMUX_CH_ROWS="$ROWS" FUZZMUX_CH_TEXT="$TEXT_ROWS" FUZZMUX_CH_META="$META" FUZZMUX_CH_LIMIT="$LIMIT" CLAUDE_DEL
...
# so state, age and project never skew a text search. The filter key switches
# to the one-row-per-session list, restricted to the sessions of the current
# text matches, where the metadata is searchable; the "all" key lifts the
# restriction. --multi is only there so select-all can hand the matches over.
ALL_KEY="$(claude_option @fuzzmux-claude-history-bind-all ctrl-t)"
PREVIEW_CMD="${SELF} --preview-line {}"
h_state="" h_age="" h_dir=""
pad_into h_state "state" 10
pad_into h_age "age" 4
pad_into h_dir "project" 38
# Same column layout as the rows: the bar follows the padded project column.
HEADER_TEXT="${h_state}  ${h_age}  ${h_dir}${BAR} conversation line     ${FZF_BIND_KEY}: sessions of these matches"
HEADER_SESSIONS="${h_state}  ${h_age}  ${h_dir}${BAR} first prompt     ${FZF_BIND_KEY}: text search   ${ALL_KEY}: all sessions"
export FUZZMUX_CH_HEADER_TEXT="$HEADER_TEXT" FUZZMUX_CH_HEADER_SESSIONS="$HEADER_SESSIONS"

FZF_ARGS=(--ansi --exit-0 --no-hscroll --multi --tiebreak=index --prompt "$PROMPT_TEXT"
  --delimiter="$BAR" --with-nth=3.. --nth=2.. --header="$HEADER_TEXT"
  --bind="${FZF_BIND_KEY}:transform:${SELF} --toggle"
  --bind="${ALL_KEY}:transform:${SELF} --all")
if [[ "$PREVIEW" == "true" ]]; then
  FZF_ARGS+=(--preview "$PREVIEW_CMD" --preview-window="$PREVIEW_WINDOW")
fi

SELECTION=$(fzf "${FZF_ARGS[@]}" <"$TEXT_ROWS") || exit 0
SELECTION="${SELECTION%%$'\n'*}" # --multi: act on the first line only
sid="${SELECTION%%"$BAR"*}"
[[ -n "$sid" && "$sid" != "-" ]] || exit 0
